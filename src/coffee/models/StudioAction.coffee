#if require? and not window?
#  _ = require "./../../../library/lodash.js"

# TODO: optimized cache for tween 
...
class Studio.Action extends cc.ActionInterval
  @properties : [
    "x"
    "y"
    "anchorY"
  ...
  _setup:()->
    ...
    for parameter, action of node.actions
      ...
      if target
        ...
        if Studio.Action.defaultProperties.color[action.parameter]?
          color = target.color
  ...
  step:(dt)->
    ...
    for action in @_actionList
      ...
      {result, isDone} = Studio.Action.Tween[action.tween](action)
...
Studio.Action.Tween.Linear = ({time, startTime, endTime, start, end})->
