...
(function(Raphael) {
    ...
    var doc = document,
        ...
        ColorPicker = function(x, y, size, initcolor, element) {
            ...
            t.disc.drag(function(dx, dy, x, y) {
                ...
                var offset = $(t.disc[0]).offset(),
                    position = $(t.disc[0]).position();
                t.setHS(x - offset.left + position.left, y - offset.top + position.top);
            }, function() {
                t.hsOnTheMove = false;
            });
            t.btop.drag(function(dx, dy, x, y) {
                ...
                var offset = $(t.btop[0]).offset(),
                    width = $(t.btop[0]).prev().attr('width') * 1,
                    left = parseInt(offset.left);

                t.setB(x - offset.left + width / 2);
            }, function() {
                t.bOnTheMove = false;
            });
    ...
    ColorPicker.prototype.docOnMove = function(dx, dy, x, y) {
        if (this.hsOnTheMove) {
            ...
            var offset = $(this.disc[0]).offset(),
                position = $(this.disc[0]).position();
            this.setHS(x - offset.left + position.left, y - offset.top + position.top);
        }
        if (this.bOnTheMove) {
            //this.setB(x - this.x);

            var offset = $(this.btop[0]).offset(),
                width = $(this.btop[0]).prev().attr('width') * 1,
                left = parseInt(offset.left);

            this.setB(x - offset.left + width / 2);
        }
    };
