(function(Raphael) {
    ...
    var pi = Math.PI,
        ...
        ColorWheel = function(x, y, size, initcolor, element) {
            ...
            t.ring.drag(function(dx, dy, x, y) {
                ...
                var offset = $(t.ring[0]).offset();
                t.setH(x - offset.left - t.size2, y - offset.top - t.size2);
            }, function() {
            ...
            t.square.drag(function(dx, dy, x, y) {
                ...
                var offset = $(t.square[0]).offset(),
                    position = $(t.square[0]).position();
                t.setSB(x - offset.left + position.left, y - offset.top + position.top);
            }, function() {
    ...
    proto.setH = function(x, y) {
        var d = Raphael.angle(x, y, 0, 0),
            rd = Raphael.rad(d);
        this.cursorhsb.attr({
            transform: "r" + [d + 90, this.size2, this.size2]
        });
        this.H = (d + 90) / 360;
        this.main.attr({
            fill: "hsb(" + this.H + ",1,1)"
    ...
    proto.docOnMove = function(dx, dy, x, y) {
        if (this.hsbOnTheMove) {
            ...
            var offset = $(this.ring[0]).offset();
            this.setH(x - offset.left - this.size2, y - offset.top - this.size2);
        }
        if (this.clrOnTheMove) {
            //this.setSB(x - this.x, y - this.y);

            var offset = $(this.square[0]).offset(),
                position = $(this.square[0]).position();
            this.setSB(x - offset.left + position.left, y - offset.top + position.top);
        }
    };
    ...
    proto.color = function(color) {
        if (color) {
            ...
            this.B = color.v;
            this.cursorhsb.attr({
                transform: "r" + [d, this.size2, this.size2]
            });
            this.main.attr({
                fill: "hsb(" + this.H + ",1,1)"
            });
